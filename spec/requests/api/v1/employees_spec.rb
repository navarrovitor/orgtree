...
RSpec.describe "Api::V1::Employees", type: :request do
  ...
  let(:ceo) { company.employees.create!(name: "CEO", email: "ceo@abcorp.com") }
  let(:tech_lead) { company.employees.create!(name: "Tech Lead", email: "tech.lead@abcorp.com", manager: ceo) }

  describe "POST /api/v1/employees/:id/assign_manager" do
    it "assigns a valid manager to an employee" do
      # Atribui o CEO como gestor do Líder Técnico
      post "/api/v1/employees/#{tech_lead.id}/assign_manager", params: { manager_id: ceo.id }

      expect(response).to have_http_status(:ok)
      # .reload garante que estamos pegando o dado mais recente do banco
      expect(tech_lead.reload.manager).to eq(ceo)
    end

    it "returns an error when trying to create a organization hierarchy loop" do
      # Tenta fazer o CEO ser liderado pelo Líder Técnico (o que cria um loop)
  ...
  describe "GET /api/v1/employees/:id/peers" do
    let!(:dev_one) { company.employees.create!(name: "Dev One", email: "dev.one@abcorp.com", manager: tech_lead) }
    let!(:dev_two) { company.employees.create!(name: "Dev Two", email: "dev.two@abcorp.com", manager: tech_lead) }
